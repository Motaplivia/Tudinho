 ...
 * HomeScreen.tsx
 * 
 * Tela principal do aplicativo que exibe:
...
import { collection, query, where, getDocs, doc, getDoc, orderBy, Timestamp, DocumentData, addDoc } from 'firebase/firestore';
...
/**
 * Função para formatar a data de uma tarefa
...
export default function HomeScreen() {
   ...
   * Executa quando o estado de autenticação muda
   */
  useEffect(() => {
  ...
  /**
   * Efeito para atualizar dados quando a tela recebe foco
   */
  useEffect(() => {
  ...
  /**
   * Função para buscar tarefas do usuário no Firestore
   */
  const fetchTasks = async (user: any) => {
    ...
    try {
      ...
      const sortTasks = (a: Task, b: Task) => {
        const urgencyOrder: Record<Task['urgency'], number> = { 
          urgente: 0, 
          alta: 1, 
          media: 2, 
          baixa: 3 
  ...
  return (
    <View style={[styles.container, isDarkMode && styles.darkContainer]}>
      {/* Cabeçalho com menu e informações do usuário */}
